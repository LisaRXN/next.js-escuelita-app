...
export default function AlumnoDetailPage() {
  ...
  const alumnoDialogRef = useRef<HTMLDialogElement>(null);
  ...
  const { data: seguimientosData } = useQuery({
    queryKey: ["seguimientos-alumno", parseInt(id)],
  ...
  const openSeguimiento = (s: SeguimientoWithAlumno | null) => {
    setSelectedSeguimiento(s);
  ...
  if (!alumno) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <Link href="/admin/alumnos" className="text-sm text-white/70 hover:text-white underline">
          Volver a la lista
        </Link>
      </div>
    );
  ...
  return (
    <>
      <main className="px-2 md:px-8 pt-10 pb-16 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-4 mb-5">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl md:text-2xl font-bold text-myzinc">
          ...
          <button
            onClick={() => { setShowAlumnoModal(true); alumnoDialogRef.current?.showModal(); }}
            className="shrink-0 text-sm px-4 py-2 border border-zinc-200 rounded-lg text-myzinc hover:bg-zinc-50 transition flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          ...
          <div className="bg-white rounded-2xl p-5">
            <h2 className="text-xs font-semibold text-mygray uppercase tracking-widest mb-4">
              Información personal
            </h2>
            <dl className="flex flex-col gap-3 text-sm">
              {[
                ...
                { label: "Sexo", value: alumno.sexo === "M" ? "Masculino" : "Femenino" },
                {
                  ...
                  value: new Date(alumno.fechaNacimiento).toLocaleDateString("es-PE", { day: "numeric", month: "long", year: "numeric" }),
                },
                {
                  label: "Fecha de matrícula",
                  value: new Date(alumno.fechaMatricula).toLocaleDateString("es-PE", { day: "numeric", month: "long", year: "numeric" }),
                },
              ].map(({ label, value }) => (
                ...
                return (
                  <div key={cal} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-2 bg-zinc-100 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${calificacionColor[cal]}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
        ...
        <div className="bg-white rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-semibold text-mygray uppercase tracking-widest">
              ...
              <span className="ml-2 font-normal text-zinc-400">({total})</span>
            </h2>
            <button
              onClick={() => openSeguimiento(null)}
          ...
          {seguimientos.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {seguimientos.map((s) => (
                <div
                  key={s.id}
                  onClick={() => openSeguimiento(s)}
                  className="flex items-center gap-3 p-3 rounded-xl border border-zinc-100 hover:bg-zinc-50 cursor-pointer transition"
                >
                  <div className="flex-1 min-w-0">
                  ...
                  <i className="fa-solid fa-chevron-right text-xs text-zinc-300"></i>
                </div>
              ))}
            </div>
          )}
      ...
      {/* Modal édition alumno */}
      {showAlumnoModal && (
        <AlumnoModal
          alumno={alumno}
          dialogRef={alumnoDialogRef}
          onClose={() => {
            setShowAlumnoModal(false);
            alumnoDialogRef.current?.close();
            queryClient.invalidateQueries({ queryKey: ["alumno", id] });
          }}
          onDelete={() => router.push("/admin/alumnos")}
        />
      )}
      ...
      {showSeguimientoModal && (
        <SeguimientoModal
          ...
          onClose={() => {
            ...
            seguimientoDialogRef.current?.close();
          }}
        />
      )}
    </>
